...
from data_contd import splitSent
...
class BLSTMEncoder(nn.Module):
    ...
    def get_word_dict(self, sentences, tokenize=True):
        ...
        sentences = [splitSent(s, splitType=self.split_type) for s in sentences]
    ...
    def get_glove(self, word_dict):
        ...
        with open(self.glove_path, encoding="utf-8") as f:
            for line in f:
                if 'charNgram.txt' in self.glove_path:
    ...
    def prepare_samples(self, sentences, bsize, tokenize, verbose):
        ...
        sentences = [[word for word in splitSent(text, splitType=self.split_type) if word in self.word_vec] for text in sentences]
...
class InnerAttentionMILAEncoder(nn.Module):
    ...
    def forward(self, sent_tuple):
        ...
        emb = torch.cat((emb1, emb2, emb3, emb4), 0)
        return emb
...
class ConvNetEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.convnet1 = nn.Sequential(
            nn.Conv1d(self.word_emb_dim, self.enc_lstm_dim, kernel_size=3,
                      stride=1, padding=1),
            nn.ReLU(inplace=True),
            )
        self.convnet2 = nn.Sequential(
            nn.Conv1d(self.enc_lstm_dim, self.enc_lstm_dim, kernel_size=3,
                      stride=1, padding=1),
            nn.ReLU(inplace=True),
            )
    ...
    def forward(self, sent_tuple):
        ...
        if self.pool_type == 'mean':
            ...
            u1 = torch.mean(sent,2)

            sent = self.convnet2(sent)
            u2 = torch.mean(sent, 2)

            sent = self.convnet3(sent)
            u3 = torch.mean(sent, 2)

            sent = self.convnet4(sent)
            u4 = torch.mean(sent, 2)
        else:
            sent = self.convnet1(sent)
...
class NLINet(nn.Module):
    def __init__(self, config):
        ...
        self.inputdim = 4* 4 * self.enc_lstm_dim if self.encoder_type == "ConvNetEncoder" else self.inputdim 
